...
try{
 ...
 await page.locator('input[type=file]').setInputFiles({name:'yomu.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(backup))});
 ...
 await fs.mkdir('test-results',{recursive:true});await page.screenshot({path:'test-results/web-game-detail.png',fullPage:true});assert.deepEqual(errors,[]);
