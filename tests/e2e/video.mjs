...
await player.evaluate(async()=>{const host=document.createElement('div');host.id='test-player';document.body.append(host);const root=host.attachShadow({mode:'open'});const v=document.createElement('video');root.append(v);const canvas=document.createElement('canvas');canvas.width=320;canvas.height=180;canvas.getContext('2d').fillRect(0,0,320,180);const stream=canvas.captureStream(10),chunks=[],recorder=new MediaRecorder(stream);const ended=new Promise(resolve=>recorder.onstop=resolve);recorder.ondataavailable=e=>chunks.push(e.data);recorder.start();const timer=setInterval(()=>canvas.getContext('2d').fillRect(0,0,320,180),30);await new Promise(r=>setTimeout(r,400));recorder.stop();await ended;clearInterval(timer);stream.getTracks().forEach(t=>t.stop());v.src=URL.createObjectURL(new Blob(chunks,{type:recorder.mimeType}));v.loop=true;v.muted=true;await v.play();});
