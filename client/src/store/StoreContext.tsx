...
import { stampChanges, mergeBlobs, type SyncBlob } from "@shared/sync-core";
import { createAccountStorage } from "@/lib/account-storage";
...
function mergeAchievements(learn: LearnState): string[] {
  return Array.from(
    new Set([...learn.achievements, ...unlockedAchievements(learn)])
  );
}
...
export function StoreProvider({ children }: { children: ReactNode }) {
  const accountStorage = useRef<ReturnType<typeof createAccountStorage> | null>(
    null
  );
  if (!accountStorage.current)
    accountStorage.current = createAccountStorage(
      localStorage,
      currentAccountScope(),
      seedState
    );
  const [box, setBox] = useState(() => {
    const owner = currentAccountScope(),
      state = accountStorage.current!.load(owner);
    return { owner, state: { ...state, learn: migrateLearn(state.learn) } };
  });
  const state = box.state;
  const setState = useCallback(
    (next: DasiState | ((s: DasiState) => DasiState)) =>
      setBox(previous => ({
        ...previous,
        state: stampChanges(
          previous.state,
          typeof next === "function" ? next(previous.state) : next
        ),
      })),
    []
  );
  useEffect(() => {
    accountStorage.current!.save(box.owner, box.state);
  }, [box]);
  const activateAccount = useCallback(
    (owner: string | null) => {
      accountStorage.current!.save(box.owner, box.state);
      const next = accountStorage.current!.load(owner),
        state = { ...next, learn: migrateLearn(next.learn) };
      setBox({ owner, state });
      return state;
    },
    [box]
  ...
  const patch = useCallback(
    (updater: (s: DasiState) => DasiState) => setState(updater),
    []
  );
  ...
  const removeItem = useCallback(
    (id: string) =>
      patch(s => ({
        ...
        lists: s.lists.map(l => ({
          ...l,
          itemIds: l.itemIds.filter(x => x !== id),
        })),
      })),
    [patch]
  );
  ...
  const clearUpdate = useCallback(
    (id: string) =>
      patch(s => ({
        ...s,
        items: s.items.map(i =>
          i.id === id ? { ...i, hasUpdate: false, updateLabel: undefined } : i
        ),
      })),
    [patch]
  );
  ...
  const removeSite = useCallback(
    (id: string) =>
      patch(s => ({ ...s, sites: s.sites.filter(x => x.id !== id) })),
    [patch]
  );
  ...
  const deleteList = useCallback(
    (id: string) =>
      patch(s => ({ ...s, lists: s.lists.filter(l => l.id !== id) })),
    [patch]
  );
  ...
  const addItemToList = useCallback(
    (listId: string, itemId: string) =>
      patch(s => ({
        ...s,
        lists: s.lists.map(l =>
          l.id === listId && !l.itemIds.includes(itemId)
            ? { ...l, itemIds: [...l.itemIds, itemId] }
            : l
        ),
      })),
    [patch]
  ...
  const removeItemFromList = useCallback(
    (listId: string, itemId: string) =>
      patch(s => ({
        ...
        lists: s.lists.map(l =>
          l.id === listId
            ? { ...l, itemIds: l.itemIds.filter(x => x !== itemId) }
            : l
        ),
      })),
    [patch]
  ...
  const reorderList = useCallback(
    (listId: string, itemIds: string[]) =>
      patch(s => ({
        ...s,
        lists: s.lists.map(l => (l.id === listId ? { ...l, itemIds } : l)),
      })),
    [patch]
  );
  ...
  const applyState = useCallback(
    (next: Partial<DasiState>) =>
      setBox(previous => {
        if (previous.owner !== currentAccountScope())
          throw new Error("account_changed");
        const merged = mergeBlobs(
          next as unknown as SyncBlob,
          previous.state as unknown as SyncBlob
        );
        const state = {
          ...previous.state,
          ...merged,
        ...
        return JSON.stringify(state) === JSON.stringify(previous.state)
          ? previous
          : { ...previous, state };
      }),
    []
  ...
  const ownerRef = useRef(box.owner);
  ownerRef.current = box.owner;
  const snapshot = useCallback(() => {
    if (ownerRef.current !== currentAccountScope())
      throw new Error("account_changed");
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        await syncProvider.push(snapshot());
        const remote = await syncProvider.pull();
        if (active && remote) applyState(remote);
      } catch {
        /* Settings exposes retry; the local cache stays authoritative while offline. */
      }
    }, 1000);
    return () => {
      active = false;
  ...
  const gradeCard = useCallback(
    (wordId: string, grade: Grade): StudyResult => {
      const { learn, result } = studyReducer(
        stateRef.current.learn,
        wordId,
        grade
      );
      patch(s => ({ ...s, learn }));
  ...
  const studyWord = useCallback(
    (wordId: string, known: boolean): StudyResult =>
      gradeCard(wordId, known ? "good" : "again"),
    [gradeCard]
  );
  ...
  const recordQuiz = useCallback(
    (correct: number, total: number) => {
      ...
      const next: LearnState = {
        ...
        streak:
          total > 0
            ? nextStreak(cur.streak, cur.lastStudied, today)
            : cur.streak,
  ...
  const setDailyGoal = useCallback(
    (goal: number) =>
      patch(s => ({
        ...s,
        learn: { ...s.learn, dailyGoal: Math.max(1, Math.round(goal)) },
      })),
    [patch]
  );
  ...
  const simulateUpdateScan = useCallback(() => {
    ...
    patch(s => {
      ...
      for (const item of s.items) {
        if (
          item.hasUpdate &&
          !s.notifications.some(n => n.itemId === item.id && !n.read)
        ) {
          found += 1;
          fresh.push({
            id: nanoid(8),
      ...
      return fresh.length
        ? { ...s, notifications: [...fresh, ...s.notifications] }
        : s;
    });
    return found;
  }, [patch]);
...
export function formatMarker(
  ...
  unit: "chapter" | "volume" | "season" | "episode" | "page" | "percent";
  n: number;
}[] {
  const parts: {
    unit: "chapter" | "volume" | "season" | "episode" | "page" | "percent";
